...
export async function POST(req: NextRequest) {
  ...
  const invite = await prisma.organizationInvitation.findUnique({ where: { token: parsed.data.token } });
  ...
  await prisma.$transaction([
    ...
    prisma.organizationInvitation.update({ where: { token: parsed.data.token }, data: { status: "ACCEPTED" } })
  ]);
